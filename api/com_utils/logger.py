...
class __APILogger:
    ...
    def __setup_logging_dirs(self):
        try:
            logging.info("Making log directory")
            ...
            # Print as logs cannot be used
            print(
                f"WARNING: COULD NOT CREATE LOGS:\
                Type {type(e)}: Message: {repr(e)}"
            )
    ...
    def __make_loggers(self) -> None:
        ...
        logging.info("Getting log modules")
        for enum in Loggers:
    ...
    def __get_logger(self, logger) -> logging.Logger:
        ...
        if type(logger) is str:
            ...
            if logger not in Loggers._member_names_:
                raise BackupException(
                    logging_message=f"Tried to get logger {logger}\
                        but didnt exist",
                )
            logger_name = logger
            ...
            raise BackupException(
                ...
                logging_message=f"Tried to get logger\
                    {repr(logger)} which was type: {type(logger)}",
        ...
        try:
            ...
            raise BackupException(
                error_code="INTERNAL_SERVER_ERROR",
                logging_message=f"Failed to get logger:\
    ...
    def log(self, logger: Loggers, message: str, logging_type: LoggingLevel) -> None:
        ...
        # Param checks
        if not isinstance(logger, Loggers) or logger is None:
            raise BackupException(
                error_code="INTERNAL_SERVER_ERROR",
        ...
        if not isinstance(logging_type, LoggingLevel) or logging_type is None:
            raise BackupException(
                error_code="INTERNAL_SERVER_ERROR",
